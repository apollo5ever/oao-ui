...
Function WithdrawInterval(token String) Uint64
10 IF ASSETVALUE(HEXDECODE(LOAD("CEO"))) != 1 THEN GOTO 100
15 SEND_ASSET_TO_ADDRESS(SIGNER(),1,HEXDECODE(LOAD("CEO")))
20 IF token != "" THEN GOTO 
30 IF BLOCK_TIMESTAMP() < LOAD("lastWithdrawlDERO") + LOAD("withdrawlIntervalDERO") THEN GOTO 100
31 SEND_DERO_TO_ADDRESS(SIGNER(),LOAD("intervalAllowanceDERO"))
32 STORE("lastWithdrawlDERO",BLOCK_TIMESTAMP())
35 RETURN 0
40 IF BLOCK_TIMESTAMP() < LOAD("lastWithdrawl"+token) + LOAD("withdrawlInterval"+token) THEN GOTO 100
41 SEND_ASSET_TO_ADDRESS(SIGNER(),LOAD("intervalAllowance"+token),HEXDECODE(LOAD(token)))
42 STORE("lastWithdrawl"+token,BLOCK_TIMESTAMP())
45 RETURN 0
100 RETURN 1
End Function
